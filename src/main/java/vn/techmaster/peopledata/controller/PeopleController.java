...
public class PeopleController {
  ...
  public Map<String, List<Person>> groupByCity() {
    return personRepo.groupPeopleByCity();
  }
  ...
  //Bài 1 - Trả về danh sách 'developer' ở các thành phố Hanoi, Saigon, Shanghai
  @GetMapping("devhanoisaigoshanghai")
  public ResponseEntity<Map<String, List<Person>>> fillterjob() {
    return ResponseEntity.ok().body(personRepo.fillterJob());
  }

  //Bài 2 - - Tìm ra thành phố có độ tuổi trung bình của lập trình viên trẻ nhất
  @GetMapping("youngestdevs")
  public ResponseEntity<List<Map.Entry<String,Double>>> developerYoungest() {
    return ResponseEntity.ok().body(personRepo.developerYoungest());
  }

  //Bài 4 - Tính mức lương trung bình của tất cả những người trên 30 tuổi
  @GetMapping("avgsalarypeopleabove30")
  public ResponseEntity<Double> tbSalary() {
    return ResponseEntity.ok().body(personRepo.tbSalary());
  }
